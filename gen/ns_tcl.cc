...
\"\n\
checkout_executable TCLSH \"/home/buaa/ns-allinone-2.35/bin/tclsh8.5\" tclsh \"\\\n\
When configured, ns found the right version of tclsh in\\\n\
/home/buaa/ns-allinone-2.35/bin/tclsh8.5\n\
but it doesn't seem to be there anymore, so\\\n\
...
\n\
if {$numIfsSimulator != \"\"} {\n\
for {set i} {$i < [$self set nifs_]} {incr i} {\n\
...
eval $self next $args\n\
\n\
$self instvar ragent_ ll_ mac_ ifq_ nifs_\n\
set t [expr {$nifs_ - 1}]\n\
\n\
set ns [Simulator instance]\n\
\n\
...
\n\
\n\
if {$numIfsSimulator != \"\" && $numIfsSimulator > 1} {\n\
set sndT [$self mobility-trace Send \"RTR\"]\n\
set namfp [$ns get-nam-traceall]\n\
if {$namfp != \"\" } {\n\
$sndT namattach $namfp\n\
...
$sndT target $ll_($t)\n\
$ragent_ add-ll $t $sndT\n\
$ragent_ add-mac-802_11 $t $mac_($t)\n\
} else {\n\
set sndT [$self mobility-trace Send \"RTR\"]\n\
set namfp [$ns get-nam-traceall]\n\
if {$namfp != \"\" } {\n\
...
if {$numIfsSimulator != \"\" && $numIfsSimulator > 1} {\n\
\n\
$ragent_ add-ll $t [$self set ll_($t)]\n\
$ragent_ add-mac-802_11 $t $mac_($t)\n\
} else {\n\
...
transitionTime_ satNodeType_ eotTrace_ phyTrace_ numifs_ workMode_ noiseChannel_\n\
\n\
...
numifs_ noiseChannel_ workMode_\n\
\n\
\n\
...
\n\
Simulator instproc change-numifs {val} {\n\
...
\n\
Simulator instproc get-workMode { } {\n\
$self instvar workMode_\n\
if [info exists workMode_] {\n\
return $workMode_\n\
} else {\n\
return \"\"\n\
}\n\
...
\n\
Simulator instproc get-noiseChannel { } {\n\
$self instvar noiseChannel_\n\
if [info exists noiseChannel_] {\n\
return $noiseChannel_\n\
} else {\n\
return \"\"\n\
}\n\
}\n\
