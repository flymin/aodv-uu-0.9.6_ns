...
#ifndef NS_PORT
    struct sockaddr_in aodv_addr;/* ϵͳ����,Structure describing an Internet socket address.  */
    struct ifreq ifr;                   //ϵͳ����,�����ӿ�����
    int i, retval = 0;
    int on = 1;
...
void NS_CLASS aodv_socket_process_packet(AODV_msg *aodv_msg, int len,
    ...
    //fprintf(stderr, "in function aodv_socket_process_packet\n");
    /* If this was a HELLO message... Process as HELLO. */
    // commented by gaoruiyuan
//    fprintf(stderr, "aodv_socket_process_packet\n");
...
//    }
    ...
    // changed by gaoruiyuan, move this function saperatly below
    // to avoid calling on hello message.

    /* Check what type of msg we received and call the corresponding
       function to handle the msg... */
    switch (aodv_msg->type) {
        case AODV_HELLO:    // added by gaoruiyuan
            //fprintf(stderr, "AODV_HELLO\n");
            hello_process((HELLO *) aodv_msg, len, ifindex);
            break;
        case AODV_HELLO_ACK:    // added by gaoruiyuan
            //fprintf(stderr, "AODV_HELLO_ACK\n");
            hello_ack_process((HELLO_ACK *) aodv_msg, len, ifindex);
            break;
        case AODV_RREQ:
            //in this version neighbor deleted by G410
