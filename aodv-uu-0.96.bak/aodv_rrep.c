...
u_int32_t retrieve_la();

/* Add channel and la to RREP */
                           ...
                           unsigned int ifindex)
{
    ...
    if (!fwd_rt) {
        ...
        fwd_rt = rt_table_insert(rrep_dest, ip_src, rrep_new_hcnt, rrep_seqno,
                                 rrep_lifetime, VALID, rt_flags, ifindex, channel, la);
               ...
               (rrep_seqno == fwd_rt->dest_seqno &&
                (fwd_rt->state == INVALID || fwd_rt->flags & RT_UNIDIR ||
                 la < fwd_rt->LA/*rrep_new_hcnt < fwd_rt->hcnt*/))) {
        ...
        fwd_rt = rt_table_update(fwd_rt, ip_src, rrep_new_hcnt, rrep_seqno,
                                 rrep_lifetime, VALID,
                                 rt_flags | fwd_rt->flags, channel, la);
    /*****************************************/
    } else {
        if (fwd_rt->hcnt > 1) {
            DEBUG(LOG_DEBUG, 0,
        ...
        if (inet_rrep) {
    	    ...
    	    if (!inet_rt)
        		rt_table_insert(inet_dest_addr, rrep_dest, rrep_new_hcnt, 0,
        				rrep_lifetime, VALID, RT_INET_DEST, ifindex, channel, la);
