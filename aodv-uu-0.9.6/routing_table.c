...
/********** Modified by Hao Hao **********/
/* Add channel and la to the routing table */
	...
	for (i = 0; i < RT_TABLESIZE; i++) {
		...
		list_foreach(pos, &rt_tbl.tbl[i]) {
			...
			if (rt->flags & RT_INET_DEST && rt->state == VALID) {
				/********** Modified by Hao Hao **********/
				// Seems that "CONFIG_FATEWAY" has been defined!
				// But I cannot find the location! xiba!
				rt_table_update(rt, gw->dest_addr, gw->hcnt, 0,
						life, VALID, rt->flags, 0, 0);
				/*****************************************/
				n++;
			}
		}
					...
					if (0) {
						DEBUG(LOG_DEBUG, 0,
						      ...
						      ip_to_str(rt2->
								dest_addr));
						/********** Modified by Hao Hao **********/
						/* This part will not be aceessed! */
						rt_table_update(rt2,
								gw->dest_addr,
								gw->hcnt, 0,
								timeval_diff
