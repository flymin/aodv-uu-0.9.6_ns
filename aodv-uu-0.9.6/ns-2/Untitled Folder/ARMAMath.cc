...
double ARMAMath::stderrData(std::vector<double> dataArray) {
    assert(this->varerrData(dataArray) >= 0);
...
double gaussrand0()
{
    ...
    if ( phase == 0 ) {
        ...
        X = V1 * sqrt(-2 * log(S) / S==0?0.00001:S);
    } else
        X = V2 * sqrt(-2 * log(S) / S==0?0.00001:S);
