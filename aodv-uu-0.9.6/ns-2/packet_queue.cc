...
/** func added by xujunpeng begin **/
void NS_CLASS packet_queue_a_init(void)
{
    INIT_LIST_HEAD(&AQ.head);
    AQ.len = 0;

#ifdef GARBAGE_COLLECT
    /* Set up garbage collector */
    timer_init(&AQ.garbage_collect_timer, &NS_CLASS packet_queue_a_timeout, &AQ);
...
void NS_CLASS packet_queue_a_destroy(void)
{
    int count = 0;
    list_t *pos, *tmp;

    list_foreach_safe(pos, tmp, &AQ.head) {
        struct q_pkt *qp = (struct q_pkt *)pos;
        list_detach(pos);
    ...
    DEBUG(LOG_INFO, 0, "Destroyed %d buffered a_packets!", count);
}

/* Garbage collect packets which have been queued for too long... */
int NS_CLASS packet_queue_garbage_a_collect(void)
{
    int count = 0;
    list_t *pos, *tmp;
    struct timeval now;
...
void NS_CLASS packet_queue_a_add(Packet * p, struct in_addr dest_addr)
{
    struct q_pkt *qp;
    struct hdr_ip *ih;
    struct hdr_cmn *ch = HDR_CMN(p);
    ...
    if (AQ.len >= MAX_QUEUE_LENGTH) {
        DEBUG(LOG_DEBUG, 0, "MAX Queue length! Removing first packet.");
        if (!list_empty(&AQ.head)) {
            qp = (struct q_pkt *)AQ.head.next;

            list_detach(AQ.head.next);
    ...
    if (qp == NULL) {
        fprintf(stderr, "Malloc_a failed!\n");
        exit(-1);
    }
    ...
    list_add_tail(&AQ.head, &qp->l);

    AQ.len++;

    DEBUG(LOG_INFO, 0, "buffered_a pkt to %s uid=%d qlen=%u",
          ip_to_str(dest_addr), ch->uid(), AQ.len);
}

int NS_CLASS packet_queue_set_a_verdict(struct in_addr dest_addr, int verdict)
{
    int count = 0;
    rt_table_t *rt, *next_hop_rt, *inet_rt = NULL;
    list_t *pos, *tmp;
...
#define ARP_DELAY 0.005
    ...
    if (rt && rt->state == VALID &&
        ...
        DEBUG(LOG_INFO, 0, "SENT %d packets_A to %s qlen=%u",
              count, ip_to_str(dest_addr), AQ.len);
    } else if (verdict == PQ_DROP) {
        DEBUG(LOG_INFO, 0, "DROPPED %d packets_A for %s!",
              count, ip_to_str(dest_addr));
    }
