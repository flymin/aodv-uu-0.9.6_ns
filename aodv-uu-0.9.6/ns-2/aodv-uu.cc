        ...
        */
        ...
        if (nIfaces) {
            for (int i = 0; i < nIfaces; ++i) {
				...
				else if(aodv_msg->type == AODV_HELLO){
					//gaoruiyuan changed
					((HELLO *) aodv_msg)->channel = i;
					printf("16071064 SEND HELLO: curnode = %d, orig = %d, dest = %d, channel = %d\n",
                	DEV_IFINDEX(ifindex).ipaddr.s_addr,
                	((HELLO*)aodv_msg)->orig_addr, ((HELLO*)aodv_msg)->dest_addr,
                	((HELLO*)aodv_msg)->channel);
				}
		...
		else if(aodv_msg->type == AODV_HELLO_ACK) {
			channel = ((HELLO_ACK *)aodv_msg)->channel;
			// gaoruiyuan added
		}
		/*****************************************/
...
int NS_CLASS command(int argc, const char *const *argv)
{
	...
	if (argc == 4) {
        ...
        if (strcasecmp(argv[1], "add-mac-802_11") == 0) {
            int temp_ = atoi(argv[2]);
