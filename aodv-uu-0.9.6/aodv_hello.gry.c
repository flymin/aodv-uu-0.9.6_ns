...
/*added by gaoruiyuan*/
#define Marcov_K 2  //定义马尔科夫阶数, gaoruiyuan added
static int all_status[2^Marcov_K];
#define ALPHA   1
#define BETA    1
...
void NS_CLASS cal_cost(void *arg){
    ...
    float prob;
    int *array, len;
    int flag, arma_predict;
    int i, channel, channel_num, k_status, cur_status;
    int N_hello, R_ack, BandWidth = 1, ETT;
    ...
    for (i = 0; i < RT_TABLESIZE; i++) {
        ...
        list_foreach(pos, &rt_tbl.tbl[i]) {
            ...
            channel_num = nIfaces? nIfaces:1;
            for(channel = 0; channel < channel_num; channel++) {
                /*Step 1 ETT*/
                list_t *item;
                N_hello = 0;
                list_foreach(item, &rt->hello_send_list) {
                    hs = (hello_sended_t *) item;
                    msg_time = &hs->time;
                    timediff = timeval_diff(msg_time, &now);
                    if (timediff < 0) { break; }
                    hello_msg = &hs->msg;
                    if(hello_msg->channel == channel){N_hello++;}
                }
                R_ack = 0;
                list_foreach(item, &rt->hello_ack_list) {
                ...
                ETT = R_ack / (N_hello * BandWidth);

                /*Step 2 ARMA*/
                array = rt->data_link->lifetimes;
                len = rt->data_link->lifetime_len;
                arma_predict = armamain(array, len);

                /*Step 3 Marcov*/
                flag = 0;
                list_foreach(item, &rt->data_link->stability){
                ...
                prob = all_status[k_status]/flag;   //TODO current is a float, need to transfer to int for use

                /*step 4. calculate cost*/
                rt->cost = ALPHA * ETT + BETA/(arma_predict*(1-prob));
        ...
        for (i = 0; i < MAX_NR_INTERFACES; i++) {
            ...
            for (i = 0; i < RT_TABLESIZE; i++) {
                ...
                list_foreach(pos, &rt_tbl.tbl[i]) {
                    ...
                    list_add(&rt->hello_send_list, &hs->l);      //added in head, ensure latest in the front
                    rt->nhello_send++;
                }
            }
            /*added end*/
            dest.s_addr = AODV_BROADCAST;   // 255.255.255.255,广播地址
...
void NS_CLASS hello_process(HELLO *hello, int rreplen, unsigned int ifindex) {
	...
	fprintf(stderr,"reveive a hello msg with dst:%s\n", ip_to_str(hello_dest));
	...
	*/
	...
	hello_ack_send(ifindex, hello_dest, hello->channel);        //added by gaoruiyuan
