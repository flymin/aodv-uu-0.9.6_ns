...
{
    /********** Add by Hao Hao to debug **********/
    printf("16231214 GET IN RREQ_PROCESS: curnode = %d\n", DEV_IFINDEX(ifindex).ipaddr.s_addr);
    /*********************************************/
    AODV_ext *ext;
    RREP *rrep = NULL;
    ...
    /********** Add by Hao Hao to debug **********/
    printf("16231214 RREQ_PROCESS CHECK REVERSE TABLE: curnode = %d\n", DEV_IFINDEX(ifindex).ipaddr.s_addr);
    /*********************************************/
    ...
    ******************************************/
    ...
    /* Compare RREQ's LA to determine how to proccess the RREQ */
    struct rreq_record *ret = NULL;
    if ((ret = rreq_record_find(rreq_orig, rreq_id)) != NULL && la >= ret->LA)
        return;
    
    ret = rreq_record_insert(rreq_orig, rreq_id);
    ret->LA = la;   //Add la into the record table
    /*****************************************/
    ...
    /********** Add by Hao Hao to debug **********/
    printf("16231214 RREQ_PROCESS CHECK FORWARD TABLE: curnode = %d\n", DEV_IFINDEX(ifindex).ipaddr.s_addr);
    /*********************************************/
	   ...
	   /* Subnet locality decision */
    	...
    	case HOST_INET:
    	    ...
    	    rrep = rrep_create(0, 0, 0, DEV_IFINDEX(rev_rt->ifindex).ipaddr,
    			       ...
    			       ACTIVE_ROUTE_TIMEOUT, channel, 0);
            /*****************************************/
    ...
    if (rreq_dest.s_addr == DEV_IFINDEX(ifindex).ipaddr.s_addr) {
        ...
        if (fwd_rt && fwd_rt->state == VALID && !rreq->d) {
            ...
            if (fwd_rt->flags & RT_INET_DEST) {
                ...
                /********** Modified by Hao Hao **********/
                /* This part will not be accessed! Do whatever you want! */
...
/********** Modified by Hao Hao **********/
// Do not add la through this function, add la into rt_table in the caller of this function
...
/********** Modified by Hao Hao **********/
u_int32_t NS_CLASS retrieve_la() {
    return 1;   // Assume the cost of transmit message between two node is 1
}
/*****************************************/
